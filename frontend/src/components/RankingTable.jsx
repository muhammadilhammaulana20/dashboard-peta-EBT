...
export default function RankingTable({ provStats }) {
  ...
  return (
    <section id="ranking" className="w-full py-16 px-6 lg:px-12 xl:px-16" style={{ background: '#0e1626' }}>
      <div className="max-w-screen-2xl mx-auto">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          <button onClick={() => setFilter('')}
            className={`px-3.5 py-1.5 rounded-full text-xs font-medium border transition cursor-pointer ${
              !filter ? 'bg-teal-500 text-navy-900 border-teal-500 font-semibold' : 'bg-white/5 text-navy-300 border-white/10 hover:border-teal-500/40 hover:text-teal-400'
          ...
          {provStats?.map(p => (
            <button key={p.provinsi} onClick={() => setFilter(p.provinsi)}
              className={`px-3.5 py-1.5 rounded-full text-xs font-medium border transition cursor-pointer whitespace-nowrap ${
                filter === p.provinsi ? 'bg-teal-500 text-navy-900 border-teal-500 font-semibold' : 'bg-white/5 text-navy-300 border-white/10 hover:border-teal-500/40 hover:text-teal-400'
              }`}>{p.provinsi}</button>
          ))}
        </div>
        ...
        <div className="relative max-w-md mb-6">
          ...
          <input type="text" placeholder="Cari desa, kecamatan, kabupaten..." value={searchInput} onChange={e => onSearchChange(e.target.value)}
            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-navy-100 text-sm placeholder:text-navy-500 focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500/50 transition" />
        </div>
                  ...
                  result.data.map((v, i) => {
                    ...
                    return (
                      <tr key={v.id} onClick={() => handleRowClick(v)}
                        className={`border-t border-white/5 hover:bg-white/5 transition cursor-pointer ${isTop3 ? 'bg-teal-500/5' : ''}`}>
                        <td className="px-4 py-3 whitespace-nowrap">
                          {isTop3 ? (
                            <span className={`inline-flex items-center justify-center w-7 h-7 rounded-lg text-xs font-bold ${
                              globalIdx === 0 ? 'bg-teal-500/20 text-teal-400 border border-teal-500/40' :
                              globalIdx === 1 ? 'bg-slate-300/10 text-slate-300 border border-slate-300/30' :
                              'bg-teal-300/10 text-teal-300 border border-teal-300/30'
                            }`}>{v.ranking_ahp}</span>
                          ) : (
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                            ...
                            v.idm_status === 'BERKEMBANG' ? 'bg-teal-500/10 text-teal-400 border border-teal-500/20' :
                            v.idm_status === 'TERTINGGAL' ? '' :
                            'bg-red-500/10 text-red-400 border border-red-500/20'
                          }`}
                          style={v.idm_status === 'TERTINGGAL' ? { background: 'rgba(139,126,110,0.1)', color: '#8b7e6e', border: '1px solid rgba(139,126,110,0.2)' } : {}}
                        >{v.idm_status}</span>
                        </td>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            <span className={`font-extrabold text-base ${v.skor_ahp >= 60 ? 'text-green-400' : v.skor_ahp >= 40 ? 'text-teal-400' : 'text-navy-400'}`}>
                              {v.skor_ahp?.toFixed(1)}
                            </span>
                            <div className="w-12 h-1.5 rounded-full bg-white/10 overflow-hidden">
                              <div className={`h-full rounded-full ${v.skor_ahp >= 60 ? 'bg-green-500' : v.skor_ahp >= 40 ? 'bg-teal-500' : 'bg-white/20'}`}
                ...
                for (let p = start; p <= end; p++) {
                  pages.push(
                    <button key={p} onClick={() => setPage(p)}
                      className={`px-2.5 py-1 rounded cursor-pointer ${p === page ? 'bg-teal-500/20 text-teal-400 font-bold' : 'bg-white/5 hover:bg-white/10'}`}>{p}</button>
                  )
                }
